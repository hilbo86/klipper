...
class LinuxFan:
    ...
    def _handle_ready(self):
        ...
        reactor.register_timer(self.callback,
                               reactor.monotonic()+HOST_REPORT_TIME)
...
class Fancontrol:
    def __init__(self, configfile, target):
        ...
        linear_b = (self.off_below*self.max_temp-self.max_power*self.min_temp)\
    ...
    def read_config(self, config_file):
        ...
        # (MINTEMP, MAXTEMP, MINSTART, MINSTOP, MINPWM, MAXPWM, AVERAGE)
        ...
        for pwm_key, temp_val in fctemps_map.items():
            ...
            entry["sym_fan"]  = self._get_sym_path(entry["fan"]) \
                if entry["fan"] else ""
            ...
            entry["abs_fan"]  = self._get_abs_path(entry["fan"], devpath_map) \
                if entry["fan"] else ""

            # Numerische Werte;
            # falls für einen pwm-Eintrag kein Wert vorhanden ist,
            # wird ein Dedaultwert übernommen.
    ...
    def write_config(self, config, filename):
        lines = []
        # Header-Kommentar
        lines.append("# Configuration file generated by klippy_fancontrol \
                     config writer, manual changes will be lost")
        ...
        # DEVPATH: Die Zuordnung erfolgt als key=value,
        # getrennt durch Leerzeichen
        devpath = general.get("DEVPATH", {})
        devpath_line = "DEVPATH=" + " ".join(f"{k}={v}" \
                                             for k, v in devpath.items())
        lines.append(devpath_line)

        # DEVNAME: In der Konfigurationsdatei heißt der Schlüssel
        # DEVNAME (nicht DEVNAMES)
        devnames = general.get("DEVNAMES", {})
        devname_line = "DEVNAME=" + " ".join(f"{k}={v}" \
                                             for k, v in devnames.items())
        ...
        for fan_key in fan_keys:
            ...
            if pwm:
                ...
                # Numerische Parameter: Falls der Eintrag vorhanden ist,
                # wird er übernommen.
        ...
        # Falls entsprechende Einträge vorhanden sind,
        # werden die Zeilen erzeugt.
        if fctemps_tokens:
    ...
    def set_fake_temp(self, value, convert=True):
        ...
        checked_value = int(max(min(value, self.max_temp*1000.),
                                self.min_temp*1000.))
        try:
...
class ControlInterpolation:
    ...
    def _generate_interpolation_coefficients(self):
        self.lin_interpol_coeff = (self.max_power - self.off_below)\
                                / (self.max_temp - self.min_temp)
        self.quad_interpol_coeff = self.lin_interpol_coeff\
                                 / (self.max_temp - self.min_temp)
    ...
    def interpolate_lin(self, temp):
        return self.off_below \
             + self.lin_interpol_coeff * (temp - self.min_temp)

    def interpolate_quad(self, temp):
        return self.off_below \
             + self.quad_interpol_coeff * (temp - self.min_temp)**2

    def interpolate_exp(self, temp):
        return self.off_below * self.exp_interpol_coeff1 \
            ** ((temp - self.min_temp) / self.exp_interpol_coeff2)
...
class FanLinearization:
    def __init__(self, calib_points=FAN_CALIBRATION_POINTS):
        ...
        with open(calib_points, "r") as f:
            for line in f:
                ...
                self.mapping.append({'pwm': int(pwm.strip()),
                                     'rpm': int(rpm.strip())})
        ...
        self.array_interpol_coeffs = [
            {'pwr_abs':
                (self.mapping[i + 1]['pwr_abs'] - self.mapping[i]['pwr_abs'])
              / (self.mapping[i + 1]['pwr_rel'] - self.mapping[i]['pwr_rel']),
             'pwm':
                (self.mapping[i + 1]['pwm']     - self.mapping[i]['pwm'])
    ...
    # speed_in -> gewünschte Drehzahl (linear) -> speed_out (gemappt)
    def by_speed(self, speed_in):
        return self._interpolate(speed_in, target='pwr_abs',
                                 source='pwr_rel', fmt=float)

    # pwm_in -> gewünschte Drehzahl (linear) -> pwm_out (gemappt)
    def by_pwm(self, pwm_in):
        return self._interpolate(pwm_in, target='pwm',
                                 source='pwm_lin', fmt=int)
